...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($result->num_rows > 0) {
        ...
        header("location: /filmmakersystem/investor.php"); // Corrected typo
        exit;
    }
    ...
    if (empty($id) || empty($firstname) || empty($lastname) || empty($email) || empty($telephone) || empty($nationality)) {
        ...
        $sql = "UPDATE investors SET firstname='$firstname', lastname='$lastname', email='$email', telephone='$telephone', nationality='$nationality' WHERE id='$id'";
        if ($connection->query($sql) === TRUE) {
            header("location: /filmmakersystem/investorview.php");
            exit;
        } else {
