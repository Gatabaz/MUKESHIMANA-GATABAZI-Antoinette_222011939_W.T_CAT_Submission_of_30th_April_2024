...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (empty($id) || empty($firstname) || empty($lastname) || empty($email) || empty($telephone) || empty($nationality)) {
        ...
        $sql = "UPDATE filmmaker SET firstname='$firstname', lastname='$lastname', email='$email', telephone='$telephone', nationality='$nationality' WHERE id='$id'";
        if ($connection->query($sql) === TRUE) {
            header("location: /filmmakersystem/filmview.php");
            exit;
        } else {
