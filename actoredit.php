...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($result->num_rows > 0) {
        ...
        header("location: /filmmakersystem/actorview.php"); // Corrected typo
        exit;
    }
    ...
    if (empty($id) || empty($firstname) || empty($lastname) || empty($email) || empty($telephone) || empty($language)) {
        echo "All fields are required";
    } else {
        $sql = "UPDATE actors SET firstname='$firstname', lastname='$lastname', email='$email', telephone='$telephone', language ='$language' WHERE id='$id'";
        if ($connection->query($sql) === TRUE) {
            header("location: /filmmakersystem/actorview.php");
            exit;
        } else {
    ...
    <center>
        ...
        <form method="POST">
            ...
            <div>
                ...
                <div>
                    <input class="company" type="text" name="language" value="<?php echo $language ?>">
                </div>
            </div>
